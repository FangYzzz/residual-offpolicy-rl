...
transition_warmup_queue = Queue()
stop_accept_warmup = False


@app.post("/push_transition")
async def push_transition(payload: dict):
    transition_queue.put(payload)
    return {"ok": True}

@app.post("/push_warmup_transition")
async def push_warmup_transition(payload: dict):
    transition_warmup_queue.put(payload)
    if stop_accept_warmup:
        return {"ok": False}
    return {"ok": True}
...
def build_policy_payload(agent, version: int):
    payload = {
        "version": version,
    ...
    return {
        "has_update": True,
        "version": version,
        "policy_b64": base64.b64encode(buffer.read()).decode("utf-8"),
    }
...
def main(cfg: ResidualTD3DexmgConfig):
    ...
    base_policy = ResidualClient(main_host="127.0.0.1", main_port=8010, action_scaler= action_scaler, state_standardizer=state_standardizer) # todo
    ...
    if len(online_rb) < cfg.algo.learning_starts and not loaded_online_from_cache:
        ...
        while len(online_rb) < cfg.algo.learning_starts:
            ...
            try:
                data = transition_queue.get(timeout=1.0)
            except Empty:
                print("No request received, break.")
                break
                
            obs = {k: torch.tensor(v, device=device) for k, v in data["obs"].items()}
            next_obs = {k: torch.tensor(v, device=device) for k, v in data["next_obs"].items()}
            reward = torch.tensor(data["reward"], device=device)
            done = torch.tensor(data["done"], dtype=torch.bool, device=device)
            combined_action = torch.tensor(data["action"], device=device)
            info = data["info"]
    ...
    base_policy.start_online_train()
